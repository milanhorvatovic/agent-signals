...
| `synthetic/context/*` | §Rotation | the state a gap derives from but never contains — a gap is generated at delivery time from a spool it does not carry, so its derived fields are only checkable against the real inputs: `pr-comments.jsonl` holds the first available event behind the retained variant, and `pr-comments.tombstone.json` the removal the empty-source variant derives from. Deriving either from the gap's own fields would be circular |
