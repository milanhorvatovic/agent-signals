...
| `manifest/valid/*.yaml` | §Manifest | validate; `example` is compared against the manifest block in the contract itself, so the two cannot drift apart; the minimal fixtures leave optionals absent to take defaults, and `all-options` sets every one of them to a non-default value. `empty-command-element` pins the boundary: the argv *array* must be non-empty, an individual argument may be the empty string |
...
| `synthetic/context/<fixture>/*` | §Rotation | the state a gap derives from but never contains — a gap is generated at delivery time from a spool it does not carry, so its derived fields are only checkable against the real inputs, and deriving them from the gap's own fields would be circular. Keyed by fixture rather than by source, because the two variants describe two different retention states of one source: `gap/` holds the first available event and the tombstone behind the retained variant, `gap-empty-source/` the removal the empty variant derives from, plus the creation baseline that makes gapping a null cursor correct — retention that ran before the cursor existed is not loss for it, so without the baseline the fixture would equally describe a cursor that must *not* be gapped |
